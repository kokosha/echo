...
#[tauri::command]
pub(crate) async fn call_gemini_api(api_key: String, messages: Vec<Message>, model: Option<String>) -> Result<LlMResponse, String> {
    ...
    let url = format!("https://generativelanguage.googleapis.com/v1beta/models/{}:generateContent?key={}", selected_model, api_key);

    // Convert the generic Message vector into the Gemini-specific format
    let contents: Vec<GeminiContent> = messages.into_iter().map(|message| {
        GeminiContent {
            // Gemini uses model for the assistant role
            role: if message.role == "assistant" { "model".to_string() } else { message.role },
